...
def all_listings_view(request):
    ...
    for_sale_listings = SaleListing.objects.filter(is_listed=True, expiration_date__gt=timezone.now(), sold=False)
...
def sold_listings(request):
    ...
    listings = SaleListing.objects.filter(sold=True,
                                          expiration_date__gt=timezone.now())
    ...
    template = 'listings/sold_listings.html'
